...
pbmc <- readRDS("3.4_regulonAUC_monkeySIV.Rds")## get AUC matrix
pbmc <- getAUC(pbmc)#
```
...
## the first step for comparison
pbmc_logtransform <- pbmc_data
...
pbmc_mtx <- bernoolize(pbmc)#
pbmc_AUCcell = CreateSeuratObject(pbmc_mtx)

mean_auc <- colMeans(df[, 1:56])
expected_variance <- mean_auc * (1 - mean_auc)

# See which regulons have highest "binomial variance"
selected_regulons <- names(sort(expected_variance, decreasing = TRUE))[1:20]

pbmc_logtransform <- RunPCA(pbmc_AUCcell, features = selected_regulons, verbose = FALSE) 
pbmc_logtransform <- RunUMAP(pbmc_logtransform,dims = 1:20, verbose = FALSE)
## correct
